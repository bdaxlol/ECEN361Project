    ...
    { //Serial.println("here");
      ...
      { 
        ...
        if(AngleAvg < 40 || AngleAvg > -40){  
          //ping library call
          echo_duration = sonar.ping_cm();
          Serial.print("Ping: ");
          Serial.print(echo_duration); // Send ping, get distance in cm and print result (0 = outside set distance range)
          Serial.println("cm");
          int temp = Turn_Need;
          if (echo_duration > 0 && echo_duration < 25) {
            //turning function
            moving = true;
            Turn_Need = 60;
            Speed_Need = 60;
            Timer1.initialize(700000);                        // Initialise timer 1
            Timer1.attachInterrupt( sonarISR );                 // Attach interrupt to the timer service routine 
          }
      ...
      UserComunication();
      ProcessRC();//This is constantly recalled to see if RC input is coming
      MusicPlay();   
    }
...
//Routine to set values back to normal after event is triggered
void sonarISR() {
  if (moving) {
  Turn_Need = 0;
  Speed_Need = 0;
...
void CarDirection()
{
  ...
  switch(SerialPacket.m_Buffer[3])
  {
    ...
    case 0x03: Turn_Need = Speed; break; //Is this the code to turn a certain way?----------------------------------------------------------------------
